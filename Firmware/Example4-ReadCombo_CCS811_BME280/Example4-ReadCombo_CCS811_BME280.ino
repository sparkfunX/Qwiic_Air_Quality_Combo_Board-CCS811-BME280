...
void loop()
{
  if (dataAvailable()) //Check to see if CCS811 has new data (it's the slowest sensor)
  {
  ...
  else if (checkForError()) //Check to see if CCS811 has thrown an error
  {
    printError(); //Prints whatever CSS811 error flags are detected
  }
...
void getWeather()
{
  ...
  float pascals = myBME280.readFloatPressure();
